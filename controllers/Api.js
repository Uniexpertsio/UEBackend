...
API.get.mongooseRes = async (req, res) => {
  res.json({
    ...
    "Model.create()": {
      returns: [
        "The created object will all the key value pairs",
        "Goes in catch if any error are encountered.",
      ],
    },
    "Model.find()": {
      returns: [
        "Empty array [] when no results found",
        "Array of objects when it find results",
      ],
    },
    "Model.findOne()": {
      returns: [
        "null when no result found",
        "Document object when it finds the match",
      ],
    },
    "Model.updateOne()": {
      returns: [
        "An object like this when modified: { n: 1, nModified: 1, ok: 1 }",
        "If nothing is modified: { n: 0, nModified: 0, ok: 1 }",
      ],
    },
    "Model.deleteOne()": {
      returns: [
        "An object like this when deleted: { n: 1, ok: 1, deletedCount: 1 }",
        "If nothing is deleted: { n: 0, ok: 1, deletedCount: 0 }",
      ],
    },
  });
};
